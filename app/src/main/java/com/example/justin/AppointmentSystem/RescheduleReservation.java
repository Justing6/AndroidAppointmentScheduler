...
public class RescheduleReservation extends FragmentActivity implements OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.reschedule_reservation);
        ...
        findViewById(R.id.confirmbutton).setOnClickListener(new OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
               new HttpAsyncTask().execute(buildMove());
            }
        });
    ...
    public String buildMove() {
        Bundle extras = getIntent().getExtras();
        if (extras != null) {
            try {
                JSONObject json = new JSONObject(extras.getString("Json"));
                String url = "http://cptgschedule.ddns.net/jsonmove.cfm?event="+json.getString("event")+"&date="+returnedslot;
                Log.d("URL", url);
                return url;
    ...
    private class HttpAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                }else if(json.getString("function").equals("move")){
                    if(json.getBoolean("success")){
                        Toast.makeText(getBaseContext(), "Your appointment has been moved successfully.", Toast.LENGTH_LONG).show();
                        ...
                        Toast.makeText(getBaseContext(), "You already have an appointment at this time.", Toast.LENGTH_LONG).show();
                    }else if(!json.getBoolean("valid")) {
                        Toast.makeText(getBaseContext(), "Please choose a date under " + json.getString("maxdate") + " days in the future.", Toast.LENGTH_LONG).show();
                    }else if(json.getBoolean("error")){
                        Toast.makeText(getBaseContext(), "We're sorry, but this appointment no longer exists.", Toast.LENGTH_LONG).show();
                        Intent resultIntent = new Intent();
                        resultIntent.putExtra(PUBLIC_STATIC_STRING_IDENTIFIER, "supertrue");
                        setResult(Activity.RESULT_OK, resultIntent);
                        finish();
                    }else{
                        Toast.makeText(getBaseContext(), "We're sorry, but this time has filled. The next open time has been displayed.", Toast.LENGTH_LONG).show();
                        new HttpAsyncTask().execute(FINDID+pickeddate+'T'+pickedtime);
                    }
                }else if (json.getString("function").equals("lookup")) {
                    if(json.getBoolean("found")){
                        Intent i = new Intent(RescheduleReservation.this, DisplayInfo.class);
